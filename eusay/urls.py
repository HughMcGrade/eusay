...
urlpatterns = patterns('',
    ...
    url(r'^add_user/', views.add_user), # TODO: remove this, maybe?
    url(r'^get_users/', views.get_users),  # TODO: remove this, since it's for debugging
    url(r'^$', views.index),
    url(r'^submit/', views.submit),
    url(r'^proposal/(?P<proposalId>\d+)/(?P<slug>[\w-]*)', views.proposal,
        name="proposal"),
    url(r'^tag/(?P<tagId>\d+)/(?P<slug>[\w-]*)', views.tag, name="tag"),
    url(r'^user/(?P<slug>[\w-]+)', views.profile, name="user"),
    url(r'^vote_proposal/(?P<vote_request_type>(up|down|get))/(?P<proposal_id>\d+)', views.vote_proposal),
    url(r'^vote_comment/(?P<vote_request_type>(up|down|get))/(?P<comment_id>\d+)', views.vote_comment),
